...
For instance, the \coqinline{nat} constructors follow the below rules:

\vspace{-2ex}
\begin{mathpar}
  \inferrule*
    {~}
    {\Gamma \vdash \text{O} : \text{Nat}^{\hat{s}}}
  \and
  \inferrule*
    {\Gamma \vdash n : \text{Nat}^s}
    {\Gamma \vdash \text{S} ~ n : \text{Nat}^{\hat{s}}}
\end{mathpar}
...
\begin{mathpar}
  \inferrule*[]
    {\Gamma (f:t^\upsilon) \vdash e: t^{\hat{\upsilon}}}
    {\Gamma \vdash \text{(co)fix} ~ f : t := e : t^s}
\end{mathpar}
