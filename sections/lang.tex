...
The free size variables of a size substition $\rho$ are only those of the sizes that $\rho$ maps \emph{to}.
...
We define $\rhd$ as the congruent closure of the reductions, $\rhd^*$ as the reflexive--transitive closure of $\rhd$,
and convertibility ($\approx$) as the confluent congruent closure of $\rhd^*$ up to $\eta$-convertibility.
We refer the reader to previous work~\citep{cic-hat-minus, cic-hat, cc-hat-omega} and the Coq manual in particular~\citep{coq} for precise details and definitions.

\input{figures/reduction.tex}

In the case of \deltaDeltareduction, where the variable has a size substitution annotation, we define additional Rules \refnorule{delta} and \refnorule{Delta}.
