...
In this section, we introduce and examine the syntax and judgements of \lang,
...
It is a standard CIC with expressions (or terms) consisting of Coq-style universes, dependent functions, definitions, \coinductives, case expressions, and mutual \cofixpoints.
...
The overline $\seq{\ph}$ denotes a sequence of syntactic constructions.
...
Mutual cofixpoints $\fix{m}{f_k}{t_k^*}{e_k}$ are similar, except cofixpoint definitions don't need $n_k$,
...
$\seq{\vec{\assm{\app{I_i}{\Delta_p}}{\prodctx{\Delta_i}{U_i}}}} \coloneqq
\seq{\vec{\assm{c_j}{\prodctx{\Delta_j}{\app{I_j}{\dom{\Delta_p}}{\vec{t}_j}}}}}$
consists of the following:
\begin{itemize}
  \item $I_i$, the names of the defined \coinductive types;
  \item $\Delta_p$, the \textit{parameters} common to all $I_i$;
  \item $\Delta_i$, the \textit{indices} of each $I_i$;
  \item $U_i$, the universe to which $I_i$ belongs;
  \item $c_j$, the names of the defined constructors;
  \item $\Delta_j$, the arguments of each $c_j$;
  \item $I_j$, the \coinductive type to which $c_j$ belongs; and
  \item $\vec{t}_j$, the indices to $I_j$.
\end{itemize}

Note that $I_j$ is the \coinductive type of the $j$th constructor, \emph{not} the $j$th \coinductive in the sequence $I_i$.
...
and similarly for $\new{\rho s}$ and $\new{\rho \Gamma}$ for every term in $\Gamma$.
...
\iffalse % TODO: Define \whnf as it comes up in the algorithm.
...
In Rules \refnorule{ind}, \refnorule{constr}, and \refnorule{case}, we use $i$ to range over the number of \coinductive types in a single mutual \coinductive definition, $j$ to range over the number of constructors of a given \coinductive type, $k$ for a specific index in the range $\vec{\imath}$, and $\ell$ for a specific index in the range $\vec{\jmath}$.
In Rules \refnorule{fix} and \refnorule{cofix}, we use $k$ to range over the number of mutually-defined \cofixpoints and $m$ for a specific index in the range $\vec{k}$.
...
In \refrule{constr}, the type of a constructor is a function type from its parameters $\Delta_p$ and its arguments $\Delta_\ell$ to its \coinductive type $I_\ell$ applied to the parameters and its indices $\vec{t}_\ell$.
...
\end{align*}
It has a single parameter $A$ and $\Succ ~ n$ corresponds to the index $\vec{t}_j$ of the constructor's inductive type.
...
\begin{itemize}
    ...
    \item The \textbf{motive} $P$.
      ...
      (This presentation follows CIC, but differs from that by~\citet{cic-hat-minus, cic-hat-l, cc-hat-omega}, where the case expression contains a return type in which the index and target variables are free and explicitly stated, in the syntactic form $\vec{y}.x.P$.)
    ...
    \item The \textbf{branches} $e_j$.
      ...
      The type of each branch, listed in \autoref{fig:metafunctions}, is the motive $P$ applied to the indices $\vec{t}_j$ of that constructor's type and the constructor itself applied to the parameters and its arguments.
      ...
      The parameter variables in $\Delta_j$ and $\vec{t}_j$ are similarly bound to the parameters $\vec{p}$ of the target.
\end{itemize}
