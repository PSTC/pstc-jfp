...
\begin{enumerate}
  \item If $\upsilon \pos t$, then $\gg \vdash \rho_1 t \leq \rho_2 t$ for some environments $\Gamma_G, \Gamma$; and
  \item If $\upsilon \neg t$, then $\gg \vdash \rho_2 t \leq \rho_1 t$ for some environments $\Gamma_G, \Gamma$.
\end{enumerate}
\end{lemma}

\begin{proof}[{[Sketch]}]
By mutual induction on the positivity and negativity rules in \autoref{fig:posneg}.
\todo: What about the environments that need to be constructed?
\end{proof}

\begin{lemma}\label{lem:subtyping-posneg}~\\[-4ex]
\begin{enumerate}
  \item If $\Gamma_G, \Gamma \vdash t \leq \subst{t}{\upsilon}{\succ{\upsilon}}$ then $\upsilon \pos t$.
  \item If $\Gamma_G, \Gamma \vdash \subst{t}{\upsilon}{\succ{\upsilon}} \leq t$ then $\upsilon \neg t$.
\end{enumerate}
\end{lemma}

\begin{proof}[{[Sketch]}]
By induction on the subtyping rules in \autoref{fig:subtyping}.
\todo: What about subtyping relations that hold by convertibility?
\end{proof}
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    This follows from \refrule{var-def} if we can show that $\rho' \Gamma_{\Psi_1} \vdash \rho' (\rho e) : \rho' (\rho t)$ holds.
    ...
    so we are unable to apply \refrule{var-def}!
    \stuck.
    ...
    However, we cannot apply \refrule{const-def} because we do not have that $\WF{\Gamma_G, \Gamma_{\Psi}}$ holds.
    \stuck.
    ...
    \item \refrule{a-fix}.
      ...
      Then by \autoref{lem:subtyping-posneg}, the last subtyping judgement implies that $\upsilon_i \pos \rho'u_k$.
