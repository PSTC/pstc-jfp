...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \end{displaymath}
    Let $\rho' \vDash C_2$.
    We must show that $\Gamma_G, \rho' \Gamma_{\Psi_1} \vdash \rho' x^\rho : \rho' (\rho t)$ holds.
    Since $\vec{\upsilon'_i}$ are not in $\Gamma_{\Psi_1}$, applying $\rho$ to $\Gamma_{\Psi_1}$ would have no effect,
    so we can rearrange the judgement to read $\Gamma_G, (\rho' \circ \rho) \Gamma_{\Psi_1} \vdash x^{\rho' \circ \rho} : (\rho' \circ \rho) t$.
