...
\begin{lemma}
$\rho \vDash \subst{C}{\upsilon}{r}$ iff $\rho \circ \subst{}{\upsilon}{r} \vDash C$.
\end{lemma}

\begin{proof}
\begin{align*}
  \rho \vDash \subst{C}{\upsilon}{s}
  &\Leftrightarrow \forall s_1 \sqsubseteq s_2 \in C, \rho (\subst{s_1}{\upsilon}{r})\sqsubseteq \rho (\subst{s_2}{\upsilon}{r}) \\
  &\Leftrightarrow \forall s_1 \sqsubseteq s_2 \in C, (\rho \circ \set{\upsilon \mapsto r})(s_1) \sqsubseteq (\rho \circ \set{\upsilon \mapsto r})(s_2) \\
  &\Leftrightarrow \rho \circ \set{\upsilon \mapsto r} \vDash C
\end{align*}
\end{proof}
...
Let $\Sigma$ be a fixed, well-formed signature, $\Gamma_G$ a global environment,
and suppose $\WF{\Sigma, \Gamma_G, \square}$ holds.
\begin{enumerate}
  \item Let $C$ be a constraint set, $\Psi$ a generic environment, $e^\circ$ a bare term, and $t$ a sized term.\\
  If the following hold:
    \begin{enumerate}
      \item $\forall \rho \vDash C$, we have $\WF{\Sigma, \Gamma_G, \rho\Gamma_\Psi}$
    ...
    \end{enumerate}
    Then for every $\rho \vDash C'$, we have $\Sigma, \Gamma_G, \rho\Gamma_\Psi \vdash \rho e : \rho t$.
  \item Let $C$ be a constraint set, $\Psi$ a generic environment, and $e^\circ$ a bare term.\\
  If the following hold:
    \begin{enumerate}
      \item \label{item:sound:infer:wf} $\forall \rho \vDash C$, we have $\WF{\Sigma, \Gamma_G, \rho\Gamma_\Psi}$
      \item \label{item:sound:infer:psi} If $\Psi = \Psi_1 (C_1, x : u \coloneqq a) \Psi_2$, then $\forall \rho \vDash C_1$, we have that $\Sigma, \Gamma_G, \rho \Gamma_{\Psi_1} \vdash \rho a : \rho u$
      \item \label{item:sound:infer} $C, \Gamma_G, \Psi \vdash e^\circ \rightsquigarrow C', e \Rightarrow t$
    \end{enumerate}
    Then for every $\rho \vDash C'$, we have $\Sigma, \Gamma_G, \rho\Gamma_\Psi \vdash \rho e : \rho t$.
\end{enumerate}
\end{theorem}
...
\begin{enumerate}
  ...
  Suppose that \ref{item:sound:infer:wf}, \ref{item:sound:infer:psi}, and \ref{item:sound:infer} hold, and let $\rho \vDash C'$.
  \begin{itemize}
