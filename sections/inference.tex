...
\begin{enumerate}
  ...
  \item \label{src:2} For all $\upsilon' \in V^\neq$, $(\set{\upsilon \mapsto \rho(\tau)} \circ \rho')(\upsilon') = \rho(\upsilon')$;
  \item \label{src:3} For all $\tau' \in V^*$, $(\set{\upsilon \mapsto \rho(\tau)} \circ \rho')(\tau') \sqsubseteq \rho(\tau')$;
...
\begin{proof}[{[Sketch]}]
Let $V^\iota = \bigsqcap V^*$.
Since we add the constraints $\tau \sqsubseteq V^\iota$ and we have $\rho(\tau) = \upsilon$ and $\rho(V^*) = \succ{\upsilon}^{n}$ for some $n$,
$\rho(\tau) \sqsubseteq \rho(V^\iota)$ holds.
Next, for all size variables $\upsilon' \in V^-$ involved in a negative cycle, we add $\infty \sqsubseteq \upsilon'$;
since being in a negative cycle transitively implies a subsizing relation $\succ{\upsilon}'^{n} \sqsubseteq \upsilon'$ for some $n$,
and $\rho \vDash C'$, the only way for $\rho(\succ{\upsilon}'^{n}) \sqsubseteq \rho(\upsilon')$ to hold is if $\rho(\upsilon') = \infty$,
which satisfies $\infty \sqsubseteq \rho(\upsilon')$.
Finally, the constraints $\infty \sqsubseteq (\bigsqcup V^\neq \cap \bigsqcup V^\iota)$ are added.
% TODO: Explain why these constraints are satisfied as well.
\end{proof}
