...
First, we use $C$ to represent a set of subsizing constraints: if $(s_1, s_2) \in C$, then we must enforce $s_1 \sqsubseteq s_2$.
...
A size expression consists of a size variable with some finite nonnegative number of successor ``hats''; instead of using a perniculous tower of carets, we write the number as a superscript, as in $\hat{\upsilon}^n$.
...
% TODO: Check the rest of this section.
