...
\begin{enumerate}
  \item $\upsilon \pos t$ iff $\gg \vdash \rho_1 t \leq \rho_2 t$ for some environments $\Gamma_G, \Gamma$; and
  \item $\upsilon \neg t$ iff $\gg \vdash \rho_2 t \leq \rho_1 t$ for some environments $\Gamma_G, \Gamma$.
\end{enumerate}
\end{lemma}

\begin{proof}[{[Sketch]}]
\emph{If direction}: By induction on the subtyping rules in \autoref{fig:subtyping}. \\
\emph{Only-if direction}: By mutual induction on the positivity and negativity rules in \autoref{fig:posneg}. \todo: What about the environments that need to be constructed?
\end{proof}
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item \refrule{a-fix}.
      ...
      Since $\Delta_m$ contains no position variables, and all position variables appear positively in $u_m$, by \autoref{lem:posneg},
