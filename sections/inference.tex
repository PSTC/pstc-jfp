...
\begin{enumerate}[label=\roman*.]
  \item \label{item:sound:wf:global} $\WF{\Sigma, \Gamma_G, \mt}$
  \item \label{item:sound:wf:local} $\forall \rho \vDash C, \WF{\Sigma, \Gamma_G, \rho\Gamma_\Psi}$
\end{enumerate}
Then the following hold:
\begin{enumerate}
  \item If $C, \Gamma_G, \Psi \vdash e^\circ \Leftarrow t \rightsquigarrow C', e$,
  then $\forall \rho \vDash C'$, we have $\Sigma, \Gamma_G, \rho\Gamma_\Psi \vdash \rho e : \rho t$.
...
Suppose \ref{item:sound:wf:global} and \ref{item:sound:wf:local} hold.
\begin{enumerate}
