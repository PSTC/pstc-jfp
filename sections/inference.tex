...
\begin{enumerate}
  ...
  \item \label{alg:reccheck:infty} Add $\infty \sqsubseteq \left(\bigsqcup V^\neq \cap \bigsqcup V^\iota\right)$ to $C$.
  \item \label{alg:reccheck:bot} Let $V^\bot = \left(\bigsqcup \set{\infty}\right) \cap V^\iota$.
  ...
  \item \label{alg:reccheck:fail} Otherwise, let $V = V^\bot \cap (V^* \setminus \set{\tau})$, and fail with $\RecCheckFail{V}$.
...
\begin{itemize}
  \item {[\autoref{alg:reccheck:smallest}]} $\tau \sqsubseteq V^\iota$: Since we have $\rho(\tau) = \upsilon$ and $\rho(V^*) = \succ{\upsilon}^{n}$ for some $n$ by assumption,
  $\rho(\tau) \sqsubseteq \rho(V^\iota)$ holds.
  \item {[\autoref{alg:reccheck:neg-cycles}]} $\infty \sqsubseteq V^-$: For all size variables $\upsilon' \in V^-$,
...
% TODO: Mention a few special cases.
\end{proof}
