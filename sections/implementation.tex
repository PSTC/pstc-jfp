...
% TODO: Figure out where to upload those.
...
% TODO From William: This is true, but not really the point,
% really we're analyzing whether the complexity is a problem in practice.
...
% TODO From William: Not an interesting hypothesis; really, we want to know this is intrinsic to total size inference,
% and not some mere slowdown because we’re bad at implementing the inference
...
More than three quarters of the compilation time in \msetlistsized is taken up by \solve and \RecCheck.
...
% TODO From William: Not very interesting; the algorithmic bounds told us that.
% The key is that the degradation almost fits perfectly the algorithmic bounds,
% so it’s not a mere implementation issue
...
% TODO From William: And this point. The algorithmic bounds dominate,
% AND the numbers are large enough in practice to matter a lot.
