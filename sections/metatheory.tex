...
Two desirable properties of type systems are strong normalization and
consistency.
%
While we do not have a proof that \lang is strongly normalizing, we
have shown that a formulation of CIC with sized types, alternative to
\lang, is strongly normalizing.
%
In this section, we give an overview of the proof and some challenges
encountered.
...
%
In order for the proof attempt to go through, we had to differ from
\lang in the following aspects:
%
\begin{itemize}
  %
  \item Size variables must be explicitly annotated, and we treat
  fixpoint recursor as binders for size variables.
  %
  ...
  %
  \item Inductive types are replaced by predicative W-types in order
  ...
  %
  \item Additionally, the strong normalization model does not include
  let statements or global or local contexts like \lang does.
  %
\end{itemize}
...
\end{align*}
%
Let $t$ be some $\Nat$ in constructor form (i.e. $t = \Succ ~ n$ or
$t = \Zero$ syntactically) where $t$ has size $\hat{s}$.
%
...
Even with a modified version of \lang, proving strong normalization
was still not easy.
%
...
%
As we all know, the sized typing rule for fixpoint recursors is
supposed to ensure termination.
%
So, in order to construct a strong-normalization model in set theory,
the idea is to take advantage of the size typing rule to give a
set-theoretic interpretation for fixpoint recursors.
%
Taking the situation of \eqref{eqn:fix-example} as example, this
section tries to show why doing so is perhaps not completely
straightforward.
