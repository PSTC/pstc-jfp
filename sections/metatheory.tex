...
the justification of $\mu$-reduction
and the ordinal interpretation of the infinite size $\infty$.
...
However, a single size variable $\upsilon$ could be annotated to more than one
inductive type.
For example, a term could contain both $I_1^\upsilon$ and $I_2^\upsilon$ where
$I_1, I_2$ are different inductive types with different closure ordinals,
and one may even be applied to the other as a parameter.
In this case, the interpretation of $\infty$ with respect to $\upsilon$ would
be the maximum of the two closure ordinals,
and in general, $\Infty(e; \upsilon)_\rho^\pi$ is computed by induction on $e$,
taking the maximum of the ordinals computed from all subterms.
