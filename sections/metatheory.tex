...
In fact, we have proof that alternative formulation of CIC with sized
types, alternative to \lang, is strongly normalizing.
%
...
\begin{itemize}
  ...
  %
  \begin{align*}
    \kw{fix}^\upsilon ~ f : \prod{n}{\Nat^\upsilon}{u} \coloneqq e
    \tag{$\star$}\label{eqn:new-fix}
  \end{align*}
  %
  where $f$ is said to \emph{bind} the size variable $\upsilon$.
  %
  This explicit binding of size variables serves the purpose of
  specifying which argument the recursion is happening on.
  %
  The corresponding typing rule then allows $\upsilon$ to be
  specialised to any size expression $s$:
  %
  \begin{mathparpagebreakable}
    ...
    { \Gamma \vdash \kw{fix}^\upsilon ~ f : \prod{n}{\Nat^\upsilon}{u} \coloneqq e :
...
its body.
%
Work under some context $\Gamma$ and put
%
\begin{align*}
  F \triangleq \kw{fix}^\upsilon ~ f : \Nat^\upsilon \to \Nat^\upsilon \coloneqq e
\end{align*}
%
and let $t$ be some $\Nat$ in constructor form, so $t$ has size
$\hat{s}$ for some size $s$.
%
...
%
\begin{align*}
  \app{F}{t} \rhd \app{\subst{\subst{e}{\upsilon}{s}}{f}{F}}{t}
\end{align*}
%
But of course the size $\hat{s}$ is unknown just from the term $t$, so
it appears that the syntax needs to be further modified, such as by
adding extra mandatory size argument to fixpoint recursors, so that we
may recover the $\hat{s}$.
%
However, as a consequence of typed reduction, this was not necessary,
because the size $\hat{s}$ can be recovered from the typing of the
term $t$:
%
\begin{align*}
  %
  \inferrule
  {\Gamma \vdash t : \Nat^{\hat{s}}}
  {\Gamma \vdash \app{F}{t} \approx \app{\subst{\subst{e}{\upsilon}{s}}{f}{F}}{t}}
  %
  \tag{$\varheartsuit$}\label{eqn:F-t-red}
  %
\end{align*}

\paragraph*{} Via this model construction, the main lesson learnt is
...
Take the situation in \eqref{eqn:F-t-red} as an example.
%
...
%
Now, consider how one would define $\Val(F)_\rho^\pi$.
%
Intuitively, we take some initial approximation of the fixpoint of the
value of $e$ and at every stage, improve the approximation by
evaluating $e$ on the prior approximation, until the approximation
cannot be further improved, which means we have reached a fixpoint of
the iteration.
...
\Nat^\upsilon)_\rho^{\subst{\pi}{\upsilon}{\alpha}}$, the usual
approach is to iterate up until the least fixpoint of the operator
%
\begin{align*}
  \varphi \triangleq
...
\end{align*}
%
starting from some the initial approximation $\varphi_0 \in D_0$ and
...
%
\begin{align*}
  \varphi_\alpha(x) = \varphi_\beta(x) \text{ for all } x < \alpha < \beta
...
should not be used to affect computation.
%
...
\end{align*}
%
By the intuitive understanding of sized typing, on input of size
$\alpha+1$, the body $e$ can only recursively call $f$ on inputs of
size at most $\alpha$.
%
And inductively, $\varphi_\alpha$ behaves identically to
$\varphi_\beta$ on inputs of size at most $\alpha$.
%
So, $e$ should not be able to distinguish between $\varphi_\alpha$ and
$\varphi_\beta$, thus ensuring the above equality.
%
...
\CIChatminus, and indeed matches the intuition of size-irrelevance.
%
