...
$$\set{\trace(f) \mid f \in \set{(a, b) \mid a \in A, b \in B_a}}$$
...
If it is $\emptyset$, then there is no possible $b$ and therefore no possible $f$,
making the interpretation of the function type itself $\emptyset$.
If it is $\set{\emptyset}$, then $b = \emptyset$, $f = (a, \emptyset)$ for some $a$,
and $\trace(f) = \emptyset$ since because $b$ is empty,
...
% The idea is that the interpretation of ill-typed terms makes no sense
% especially when considering the convertibility (equality) or subtyping (subset) of interpretations,
% so reduction, convertibility, and subtyping should all be typed to prevent senseless situations.
...
%
For example, in $e$, the size variable $\upsilon$ can be annotated to both
$\Nat^\upsilon$ and $I^\upsilon$, where $I$ is some inductive type with a
different closure ordinal than $\Nat$.
%
In fact, it turned out that computing $\Infty(e;\upsilon)_\rho^\pi$ was
much more involved than initially expected.
%
\todo: This sentence sounds like the intro of a new paragraph, but it doesn't go anywhere...
