...
%
For example, a size variable $\upsilon$ can be annotated to
both $I_1^\upsilon$ and $I_2^\upsilon$, where $I_1, I_2$ have different closure
ordinals and $I_1$ may even contain $I_2$ as a parameter.
%
In this case, the interpretation of $\infty$ would be the maximum of their closure ordinals.
%
\todo: Verify w/ Michael.
In general, $\Infty(e; \upsilon)_\rho^\pi$ is computed by induction on $e$,
taking the maximum of the ordinals computed from all subterms.
%
\iffalse
% tbh I don't think we even need any sort of concluding remark
