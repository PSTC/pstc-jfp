...
\begin{proof}[{[sketch]}]
  ...
  % Is it though?
  One notable aspect of this proof is that to handle let statements,
  we need to extend the Takahashi method to cover local environments
  as well.
  This is also observed in Section 2.3.2 of \citet{coq-coq-correct}
  and in particular we require Theorem 2.1 (Parallel Substitution) of
  \citet{coq-coq-correct} to ensure that $\delta$-reduction
  (i.e. reducing a let-expression) is confluent.
\end{proof}
