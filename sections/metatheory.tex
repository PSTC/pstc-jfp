...
unfortunately, the model requires changes to \lang that are backwards
incompatible with Coq, so we do not pursue it further.
We discuss from where these backwards incompatible changes arise for posterity.
...
\lang called \langAnother which made a series of simplifying assumptions about
...
\begin{itemize}
  ...
  \item Fixpoint type annotations require explicit size annotations
    (\ie are no longer merely position terms), explicitly abstract over a size
    variable, and are explicitly applied to a size expression.
...
We leave this line of inquiry as future work\footnote{In fact, ongoing work by
  ...
  communication, Dec. 2021).}, since we have other reasons to believe backwards
incompatible changes are necessary in \lang to make sized typing practical.

Nevertheless, we next explain where each these changes originate and why they
seem necessary for the model, as well as notable difficulties that arise in the
construction.
...
no longer true in the model, when sizes are modeled as expressions and size
...
and sized---which allows proving that functions are well defined in the model.
